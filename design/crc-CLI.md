...
- cmdSearch: adds --session NAME flag (implies server proxy),
  ...
  --no-tmp excludes tmp:// documents from results and skips the
  onlyIfTmp server check (always searches locally).
  Without --no-tmp or --session: sends onlyIfTmp probe to server.
  If server returns 204 (no tmp docs), searches locally. If server
  returns results, uses those.
...
- cmdMessageNewRequest: validate FILE doesn't exist, build TagBlock
  ...
  (YYYY-MM-DD), render with heading scaffold. If stdin is not a
  terminal, read body until lone `.` on a line and append after
  scaffold. Write file.
- cmdMessageNewResponse: same pattern as new-request with ark-response tags.
  ...
  today. Stdin body reading works identically.
