...
- Recall with `opts.Propose=true` runs the derivation pass on the substrate's full scored chunk set — internally retaining tagless chunks for derivation (forcing `KeepTagless=true` for the derivation chunk set) while still applying the caller's effective `KeepTagless` to the surfaced result. **Inverted (R3068):** the pass no longer writes bbolt RC directly; each surviving candidate is authored as an `@ext-candidate` file tag, and the indexer derives RC on reindex. The caller's surfacing is unaffected — `--propose` does not change which chunks appear in the result stencil.
- derivationPass(txn *bbolt.Tx, scored []ChunkScore) error: internal step. Reads `maxED = Store.MaxEDSerial()` once for the batch (R2669); RF freshness gate unchanged (R3072). For each scored chunk: read RF[chunkid]; if `RF >= maxED`, skip derivation (R2669). Otherwise cosine-compare EC against ED (+EV), take top-N (`derivationK` default 10) (R2670), filter already-attached (F-record probe) (R2671), filter ext-routed tagnames (bare-name via ExtMap) (R2672), filter net-rejected `(chunk, tag)` via `ExtMap.rejectByChunk` (R3070, superseding the `Store.HasDerivedRejection` key lookup of R2673). For each survivor, **author `@ext-candidate` via `DB.CandidateExtTag`** (a dup bumps `@count`, R3075) instead of `Store.WriteDerivedProposal`; then `Store.WriteDerivedFreshness(chunkid, maxED)` stamps the chunk (R3068, R3072). Authoring routes through the closure actor; avoid re-entrant `SyncVoid` if this already runs inside a `Sync` closure. **Then synchronously materialize** (R3076): still on the actor, reindex each **distinct** touched mirror once via `DB.syncOnePath` (deduped by target file — authoring for all proposals precedes the sync, and a mirror with several proposals reindexes once), so the RC records derive and `enrichProposedTags` surfaces the proposals in the same call. Embedding stays deferred to `BatchEmbedChunks`, so the sync cost is only FTS + tag + derive of tiny mirrors.
- enrichProposedTags(result *RecallResult, chunkSimilarities map[uint64][]proposalSim) error: for each surfaced chunk, read `ExtMap.candidateSourcesByChunk[chunkID]` → source_tvids, recover each `(tagname, value)` via `TvidMap.Resolve` + `ParseExtTarget`, and populate `RecalledChunk.ProposedTags` ordered by similarity descending (R2684, R2685, R2686, R3067). Similarity sources: for chunks the pass derived this call, scores pass through `chunkSimilarities`; for fresh-skip chunks, compute on demand (max cosine across the tag's def files) (R2685). The stencil renderer omits the line when `ProposedTags` is empty (R2684).
