...
## Does
### Find-Connections Substrate (substrateOp)
- substrateOp: the operation object (Monadic Wrapper) for one normal-mode
  find-connections computation. Holds a DB read view bound to a private
  fts.Copy() (l.db.withFTS) as its sole fts door, so the pass's cache
  reads (FileIDPaths, FileInfoByID, SearchFuzzy) never race the write
  actor's InvalidateCaches. runSubstrate builds it (newSubstrateOp) and
  calls compute → forInput ×N. Embedding and the bbolt-only vector
  search (SearchChunks) stay on the live Librarian. (R2579, R2580, R2581, R3163)
...
- BatchEmbedChunks(): scan files in priority order, read each file's
  ...
  its chunk list's EC records (R1848). Logs embedded, skipped,
  deduped, and tag-only counts (R1864). Priority sort, tier bucket dispatch, EC
  ...
  keep the original content (R2913). A chunk that cannot produce a vector
  (too large for every tier, or stripped to empty) gets a nil sentinel EC
  record so the Pass 1 check treats it as handled and it is not re-queued
  every reconcile (R3004). Strips every text strategy including
  pdf (a pdf chunk's content is extracted text). Logs the path and id of
  any chunk containing a NUL byte so a malformed source file names itself
  (R2992) — the replacement itself happens at the tokenize chokepoint.
  Called post-reconcile after BatchEmbed. (R1609-R1617, R1846-R1848,
  R1862-R1864)
- tokenize chokepoint: every embedding path (single embed, batch embed,
  token count) tokenizes through one ark-local helper that replaces NUL
  bytes (`\x00`) with spaces before calling yzma's Tokenize — a NUL ends
  the C string early and aborts the process, and yzma swallows the error;
  a space (not deletion) keeps adjacent tokens from fusing. The single
  chokepoint means no caller can pass a NUL through (R2991).
...
- WaitForConnectionsRequest(timeout time.Duration) bool: block
  ...
  entry. (R2321)
- finalizeConnectionsDoc(id, mutate func(*ConnectionsRecord), body...):
  the single door for all three terminal transitions. Stops the timeout
  timer and elapsed-ticker eagerly, then — INSIDE one write-actor closure —
  checks/sets Done, applies mutate (the transition's own fields), renders,
  and writes. Because the write actor serializes closures (R1067), the
  check-and-set of Done is atomic (first terminal write wins, later ones
  discard) and Done is observable only once the write is in flight (so a
  poller that sees Done can't tear the fts down before WaitWritesIdle drains
  the write — the observe-before-durable race). (R3164)
- SetConnectionsResult(id string, result *ConnectionsResult) error:
  ...
  delegates to finalizeConnectionsDoc with the completed-status mutation
  and the rendered body (@connections-status: completed +
  @connections-completed:<RFC3339> + @connections-progress: done).
  Late call after terminal state: log and return nil. (R2317, R2329, R2330, R2333, R3164)
- SetConnectionsError(id string, message string): delegates to
  finalizeConnectionsDoc with the errored-status mutation
  (@connections-status: errored + @connections-error:<message> +
  @connections-completed + done). Idempotent on terminal state. (R2318,
  R2329, R2333, R3164)
...
### Recall (Phase 2B)
- recallOp: the operation object (Monadic Wrapper) for one Recall computation, mirroring substrateOp. `newRecallOp` binds a DB read view to a private `fts.Copy()` (`l.db.withFTS`) as its sole fts door, so the pass's Go-side cache reads — `normalizeInputs`' `FileIDPaths`, the `FileInfoByID` behind `ChunkInfo`/`FileStrategy`/`resolveSearchEntryChunkID`, `SearchFuzzy`'s path resolution, and the `NewChunkCache` content reads (including the `chatFunnel` sub-chunk pass, an `op` method) — never race the write actor's `InvalidateCaches`. Embedding, the bbolt-only vector search (`SearchChunks`), and store (V/D/EC) reads stay on the live Librarian. `normalizeInputs` is a db-taking helper shared with `FindConnections`, which normalizes its enqueue-time inputs on a one-shot copy. (R995, R3163)
...
- Recall with `opts.Propose=true` runs the **compute-for-display** derivation pass on the substrate's full scored chunk set — internally retaining tagless chunks for derivation (forcing `KeepTagless=true` for the derivation chunk set, R2668) while still applying the caller's effective `KeepTagless` to the surfaced result. **Compute-only (R3079):** the pass computes candidates and returns them transiently for this call; it authors no `@ext-candidate`, writes no RC/RF records, and runs no synchronous materialization. The **calling agent** is the sole author of durable candidates, via `ark ext candidate` (R3081); the pass surfaces proposals for it to author. The caller's surfacing is unaffected — `--propose` does not change which chunks appear in the result stencil.
- Recall folds in the live-conversation chunk set (R3082): an optional `RecallOpts` field carrying the source-seed chunk + recent-N turn chunk IDs (watcher-populated; empty for the bloodhound seed) is added to the compute set with A66 self-exclusion **bypassed** and `tag-only` set (R2869), so these chunks earn computed proposals (surfaced for the calling agent to author) yet are never surfaced back to the reader (R2872).
- runDerivationPass(scoresMap) (map[uint64]*chunkWork, error): the compute-only proposal pass. For each scored chunk, `selectCandidates` cosine-compares EC against ED (+EV, R2911), takes top-N (`derivationK` default 10) (R2670), applies the `min_propose_similarity` floor (R2742), filters already-attached (F-record probe) (R2671), filters ext-routed tagnames (bare-name via ExtMap) (R2672), and filters net-rejected `(chunk, tag)` via `ExtMap.rejectByChunk` (R3070). Returns the surviving candidates per chunk (similarity desc) **transiently** — no `@ext-candidate` authoring, no RC/RF write, no `syncOnePath` materialization (R3079, superseding the R3068 inversion and the R3076 sync-materialize). RF freshness is retired with the durable RC cache (R2669/R3072 retired): the pass always computes for the chunks it is asked about, so a repeat `--propose` recomputes rather than skipping.
- enrichProposedTags(chunks []RecalledChunk, computed map[uint64]*chunkWork): for each surfaced chunk, populate `RecalledChunk.ProposedTags`/`ProposedTagScores` from this call's transient `computed` proposals (similarity desc, as `selectCandidates` already ordered them) — **not** from `Store.DerivedProposals`/`candidateSourcesByChunk`, which are retained as the forge-facing reader (R3067, #37). R3080 supersedes R3067's recall-path read. The stencil renderer omits the line when `ProposedTags` is empty (R2684, R2685, R2686).
- Admission-time **path-scope** filter (R3185, R3186, R3192): when
  `RecallOpts.FilterFiles` / `.ExcludeFiles` are set, a `recallScope`
  resolved once up front holds the fileIDs whose canonical path passes
  the globs — built by walking `op.db.fts.FileIDPaths()` (the Go-side
  cache the operation object already binds) and testing each path with
  the shared `Matcher` (crc-Matcher.md, R3195) in its rootless form, so
  the scope means exactly what `ark search -files` means (R3192) — the
  hunt's globs arrive already anchored, so they carry the absolute form. Candidates are then gated at the
  substrate's per-chunk admission point: resolve chunkID → fileID and
  reject anything outside the set, memoizing the verdict per chunkID so
  a chunk hit by several substrate legs pays one lookup. Filtering
  **here** rather than on the ranked result is the whole point — K is
  applied after admission, so a post-filter would let a scoped hunt
  whose global top-K fell outside the scope return nothing while looking
  identical to a hunt that genuinely found nothing (R3186).
  Injected conversation chunks (`opts.ConversationChunks`, R3082) bypass
  the scope: they are the caller's own context, not search hits (R3187).
  When positive globs were supplied and the resolved set is **empty**,
  `RecallResult.ScopeEmpty` reports it so the caller can distinguish a
  wrong glob from an empty corpus (R3188) — the substrate states the
  fact and leaves the phrasing to the caller.
...
- **Per-cell logging (R2909):** each surfaced result's originating cell +
  per-component scores ride the per-result recall records — the CLI
  stencil (`@chunk-cell` + the `@chunk-evidence-*` lines, RenderRecallChunks)
  and the watcher's curation-doc candidate block (`- cell:` / `- evidence:`,
  RecallCurationBuilder.Candidate). These durable per-fire docs are the
  tuning log; the aggregate `recall.jsonl` monitor stays per-fire and is
  unchanged.
- **Propose EV leg (R2911):** `selectCandidates` scores chunk-EC against
