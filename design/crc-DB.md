...
- CandidateExtTag(targetSpec, tag, value, insight, disposition string) error:
  author an `@ext-candidate` (proposed routing) into the mirror tree,
  carrying an optional quoted `insight` placed **first, before the
  TARGET** (no `@` sigil) and a `disposition` (internal|external, default
  external) right after the marker. The line is stamped with a **first-seen
  date** (today) that is frozen on later bumps. Append semantics: a
  differing insight *or disposition* is a new proposal line with its own
  tally — both are part of the line identity — so distinct proposals on one
  `(TARGET, tag)` are preserved. An exact-identity duplicate (TARGET, tag,
  value, insight, disposition) **increments the line's `@count`** rather than
  ...
  lose a concurrent bump. (R3053, R3075, R3090, R3092)
- AcceptExtTag(targetSpec, tag, value string) error: rewrite every
  ...
  the candidate in one file edit. The candidate's leading date, `insight`,
  and `@count` are dropped. On reindex the RC derivation drops and the X+V edge lands,
