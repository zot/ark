...
- CandidateExtTag(targetSpec, tag, value, insight string) error:
  ...
  TARGET** (no `@` sigil). Append semantics: an exact duplicate (TARGET, tag, value,
  insight) is a silent no-op; a differing insight is a new proposal
  line, so multiple insights on one `(TARGET, tag)` are preserved. Same
  resolution / mirror-path / temp+rename write as SetExtTag, on the
  candidate class. (R3053)
- AcceptExtTag(targetSpec, tag, value string) error: rewrite every
  ...
  the candidate in one file edit. The candidate's `insight` is dropped.
  Missing file/line is a silent no-op. (R3054)
- RejectExtTag(targetSpec, tag, value string) error: rewrite matching
  `@ext-candidate` line(s) to a single tag-name-only
  `@ext-judgment: TARGET @tag:`, a durable rejection, consuming the
  candidate(s) for that `(TARGET, tag)`. Missing file/line is a silent
  no-op. (R3055)
