# PtyHost
**Requirements:** R3114, R3115, R3116, R3117, R3118, R3119, R3120, R3121, R3122, R3124, R3125, R3126, R3127, R3128
...
- childEnv: build the child's environment from the server's, stripping the Claude Code session-identity markers (`CLAUDECODE`, `CLAUDE_CODE_*`, `AI_AGENT`) so the hosted `claude` comes up as a fresh top-level session, not a detected sub-session that refuses to complete a turn; credentials (`ANTHROPIC*`) and other config pass through, and `TERM` is ensured (R3127).
