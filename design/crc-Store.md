...
- AllTagsForChunk(chunkID): union of inline TagsForChunk plus
  ...
  parity. (R2344, R2351)
- AllTagsForFile(fileID): deduplicated union of AllTagsForChunk over the
  file's chunk ids (obtained through the chunksForFile resolver wired by
  SetChunkResolver). Collapses the per-chunk multiset to one file-wide set;
  returns nil when the resolver is unset (test paths) or the file has no
  chunks. Backs `ark tag chunk FILE -all` and `ark tag get FILE -all`. (R3083)
...
### Bible Book Index Records (R3214, R3221)
- bookIndexKey(source, book, chapter): `B` + source + `\0` + book + `\0` +
  decimal chapter. NUL delimits the variable-length fields, since a literal
  `0` would collide with the digits in a chapter number or a path; the source
  leads the key so two scripture sources cannot collide on the same book and
  chapter, and so a whole source's records share one scannable prefix. (R3214)
- WriteBookIndex(source, book string, chapter int, path string): write one
  record. Called per chapter by BibleChunker.FlushBookIndex after the file
  commits. (R3214)
- ReadBookIndex(source, book string, chapter int) (string, error): exact-key
  read — chapters do not span files — returning "" when absent so a
  nonexistent book or chapter resolves to nothing rather than erroring (R3180).
- PruneBookIndex(keep func(source string) bool): scan the `B` range and delete
  every record whose source fails `keep`. The reconcile sweep at config-resolve
  (R3221); the source field is read back off the key, which is why it leads.
