...
- TagFiles and TagValueFiles add a third leg: `ExtMap.OverlayTagFiles`
  / `ExtMap.OverlayTagValueFiles` contribute overlay-routed @ext
  targets that have no LMDB V record. The three sources (persistent
  LMDB, TmpTagStore overlay-direct, ExtMap overlay-routed) union
  without coordination — chunkids do not collide. (R2019)
