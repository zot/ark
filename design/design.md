...
- [ ] O156: Remaining off-actor bare-fts cache readers race InvalidateCaches — same class as O154, fixed for the substrate by the substrateOp/fts.Copy pilot (R3163) but still live elsewhere: Recall normalization + substrateChunkText + resolveSearchEntryChunkID via l.db (recall.go), the FileIDPaths callers in search.go, and BuildFetchPayload (connections.go). Migrate each onto an operation object reading through l.db.withFTS(fts.Copy()), per specs/db-concurrency.md 'Protected Resources'. Queued as PENDING #46.