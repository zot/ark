...
- [ ] O156: Remaining off-actor bare-fts cache readers race InvalidateCaches — same class as O154, fixed for the substrate by the substrateOp/fts.Copy pilot (R3163). Migrated since: BuildFetchPayload (#46-1, copy-read via withFTS) and Recall — recallOp covering Recall + chatFunnel + the shared normalizeInputs helper + FindConnections' enqueue-time normalization (#46-2, R3163). Still live: the FileIDPaths callers in the long-lived Searcher (search.go), plus the HTTP handler-wrapper conversion (srv.handler(Op{})) so the discipline is grep-auditable across the server. Migrate each onto an operation object reading through l.db.withFTS(fts.Copy()), per specs/db-concurrency.md 'Protected Resources'. Queued as PENDING #46 (#46-3).