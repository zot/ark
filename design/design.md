...
- [ ] O158: Two path-glob mechanisms with confusingly similar names: --filter-files/--exclude-files (files/status/tag/subscribe) anchor unanchored globs with **/ and match via ark.Matcher server-side, while search -files anchors to the client cwd and matches basename-or-doublestar (pathMatchesGlob). Same-looking glob, different result. bloodhound search's flags deliberately follow the -files column (the secretary's own searches must agree); the divergence is now documented in specs/cli-commands.md 'Path filters' but not reconciled. Reconciling would change shipped search behavior, so it needs Bill's call. R3192
