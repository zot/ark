...
### CRC Cards
- [ ] crc-DB.md → `db.go`, `locator.go`, `compact.go`, `util.go`
...
- [x] crc-HTTPOperation.md → `httpop.go`
...
- [x] crc-LuhmannEvents.md → `luhmann_events.go`
...
- [x] seq-http-operation.md → `httpop.go`
...
- [x] seq-bible-resolve.md → `biblechunker.go`, `db.go`
- [x] crc-PDFChunker.md → `pdfchunker.go`
- [x] crc-BibleChunker.md → `biblechunker.go`
- [x] crc-BibleRenderer.md → `bibleview.go`, `install/html/content-markdown.html`
...
- [x] seq-luhmann-events.md → `luhmann_events.go`, `cmd/ark/monitoring_cli.go`, `recall_next.go`, `server.go`
...
- [x] test-HTTPOperation.md → `httpop_test.go`
- [x] test-DateParseGuards.md → `scheduler_test.go`
- [x] test-Config.md → `config_test.go`
- [x] test-ExtMirror.md → `ext_mirror_test.go`
- [x] test-ContentView.md → `contentview_test.go`
- [x] test-ExtAnchor.md → `ext_anchor_test.go`
- [x] test-BibleChunker.md → `biblechunker_test.go`
- [x] test-VerseResolution.md → `verse_resolution_test.go`
- [x] test-BibleRender.md → `bibleview_test.go`
...
- [x] test-RecallScope.md → `recall_scope_test.go`
- [x] test-OffActorReads.md → `contentview_race_test.go`
...
- [ ] O63: No unit tests for chunked content rendering in handleContentView — **narrowed 2026-07-20** (#41 piece 4): test-ContentView.md → `contentview_test.go` now covers the markdown chunk walk, the `?range=` single-chunk view, the non-markdown chunk loop, the ranged unchunked wrapper, and the unindexed fallback, driven through the real handler. Still uncovered, each needing a fixture disproportionate to write today: PDF page aggregation (`renderPdfChunksByPage`) and PDF single-chunk preview (`renderPdfPreview`) need a real PDF; the chat-jsonl role-grouping loop (`openRoleGroup`/`closeRoleGroup`) needs chunks carrying `role` attrs, which only the JSONL chunker produces.
...
- [ ] O67: No unit tests for cmdFiles --status, --detail, percentileInts (the `matchBaseSet` half retires into Matcher under #51 — its coverage lands in test-Matcher.md / test-FilterStack.md instead)
...
- [ ] O80: No unit tests for parseFilterStack, formatFilterStack, or -parse output. **Partly discharged 2026-07-21 by #51:** `parsePathFilterStack` — the path-only subset the five non-search commands use — is covered by test-FilterStack.md → cmd/ark/main_test.go (polarity, pass-through, double-dash normalization, negative-only, anchoring idempotence, pointing-error message). What remains is the search walker itself: `parseFilterStack`'s mode vocabulary and bare-term coalescing, `formatFilterStack`, and the `-parse` round-trip.
...
- [ ] O86: No tests for @ext storage layer — test-ExtMap.md and integration coverage of multi-set V, X record CRUD, and the canonical re-resolution flow not yet written. **Narrowed 2026-07-20** (#41 Pass 2a): test-ExtAnchor.md → `ext_anchor_test.go` now covers one end-to-end slice — author an anchored `@ext`, index, Rebuild, and read the routing back through `ExtRoutingsForTargetChunk` — which is the path that would have caught the always-empty `TargetAnchor`. Multi-set V, X record CRUD, and re-resolution remain uncovered.
...
- [ ] O156: Remaining off-actor bare-fts cache readers race InvalidateCaches — same class as O154, fixed for the substrate by the substrateOp/fts.Copy pilot (R3163). Migrated since: BuildFetchPayload (#46-1, copy-read via withFTS) and Recall — recallOp covering Recall + chatFunnel + the shared normalizeInputs helper + FindConnections' enqueue-time normalization (#46-2, R3163). Also migrated: the off-actor HTTP read handlers — handleContentView and the Librarian's HandleExpandSearch each bind one withFTS(fts.Copy()) view at the top of the handler and thread it through their subtree (#46-3, R3165). PREMISE CORRECTED 2026-07-18: the Searcher's cache readers (search.go) were counted here as pending work, but they are reached only from handlers already inside Sync(srv.db, …), and InvalidateCaches runs only on the actor — so they are serialized by construction and need no migration. Raciness follows the calling goroutine, not the reader call site; the unit to inventory is off-actor ENTRY POINTS plus their reachable subtrees. SCOPED 2026-07-19 (#46-4, Bill's call): the handler-wrapper conversion is adopted **incrementally, not as a sweep**. The generic front door is built — handle(srv, someOp{}) with an HTTP-agnostic run() and semantic error classes (R3166-R3170, httpop.go, specs/http-operations.md) — with three worked examples covering the axes a new op needs: statusOp (query param + actor read), fetchOp (decoded body + a not-found class the wrapper maps to 404), and expandSearchOp (decoded body + its own fts.Copy() read view bound in init). Remaining handlers stay ordinary http.HandlerFunc and convert opportunistically as they are touched; handlers writing non-JSON bodies are permanently outside the pattern. The one-grep audit property (every srv.db use is an op binding or a bug) therefore arrives only at full adoption, and the srv.db audit is deferred until then — weighed against a 115-handler mechanical refactor across a 6.5k-line file with no known racy entry point remaining, the sweep did not earn itself. Queued as PENDING #46.
- A77: R3170 (incremental, opportunistic adoption of the operation wrapper) is a NEGATIVE/NATURAL requirement — it records a decision NOT to do something (the 115-handler mechanical sweep), so its realization is the absence of that sweep and there is no positive code site to anchor. Kept as a requirement rather than dropped because a future agent reading crc-HTTPOperation.md would otherwise read the three worked examples as an unfinished migration and 'finish' it; O156 carries the same guard. Re-examine only if adoption ever does approach completeness, at which point the srv.db one-grep audit becomes meaningful and R3170's premise expires.
- [ ] O157: A ranged view of a file with no indexed chunks emits its ark-chunk wrapper without an <ark-ext-tags> block, so @ext routings on that chunk are invisible in that one case — every chunk-walking case attaches them via chunkDiv. Pre-existing (predates the #41 piece-4 case-split, which preserved it byte-for-byte); surfaced by the extraction, which put the two paths side by side. Fix is to route the wrapper through chunkDiv, which also adds the trailing newline the current wrapper omits — a rendering-only difference, but it changes output, so it needs its own change rather than riding a behavior-preserving refactor.
- [ ] C2: crc-Server.md's wrapTagElements bullet claims it 'Skips tags already inside <ark-tag>', but the implementation (server.go) skips only matches preceded by a backtick or sitting inside a <code> element — there is no <ark-tag> containment check. Harmless today because nothing passes <ark-tag> markup into wrapTagElements: it is applied to goldmark output and to HTML-escaped text, neither of which contains one, and the per-chunk <ark-ext-tags> block is written outside it by chunkDiv. It becomes load-bearing the moment any caller embeds ext markup in content before wrapping — a routed tag VALUE containing an @word: pattern (ordinary for ark compound tags) would be re-wrapped into a nested <ark-tag> inside a <value>. The bible verse render (R3182) deliberately preserves the invariant by inserting verse ext blocks AFTER wrapTagElements rather than relying on the documented-but-absent skip. Resolve by either implementing the skip (a no-op for every current caller) or correcting the CRC bullet to describe what the code does.
- [x] O158: **THREE** path-glob mechanisms with confusingly similar names, not two (corrected 2026-07-20; the original entry generalized from the two examined during #48): (a) ark.Matcher via matchBaseSet/matchPath/ChunkStats.matcher — files, status --chunks, tag files, tag values, and [schedule] config; prefixes **/ to any unanchored pattern. (b) pubsub.go anchorGlob+doublestar — subscribe; prefixes **/ ONLY when the pattern contains no slash at all, so 'specs/**' matches nothing (see O160). (c) search.go pathMatchesGlob, preceded by client-side cwd anchoring — search -files and, by design, bloodhound hunt scopes. Measured divergence is narrow: only a bare no-slash glob ('*.md') differs between (a) and (c), and only a relative with-slash glob ('specs/**') between (a) and (b). Resolution decided 2026-07-20 (Bill): unify on (c) — cwd-anchored, '/**/*' for any-dir — switching files/status/tag/subscribe to the -with/-without + -files stack. Now owned by PENDING #51. R3192
- [ ] O159: The "tmp://" prefix is a bare literal at 7+ sites (db.go x4, scheduler.go, search.go AnchorGlobToDir, recall paths) rather than a named constant, against the project's named-constants preference. Hygiene sweep, no behavior change.
- [x] O160: BUG: 'ark subscribe --filter-files GLOB' silently matches nothing for any relative glob containing a slash ('specs/**', 'design/*.md'). pubsub.go anchorGlob only prefixes **/ when the pattern has no slash at all, so 'specs/**' is matched verbatim against absolute indexed paths and never hits. The same glob works on 'ark files'. Failure is invisible: a subscription matching nothing is indistinguishable from a quiet corpus. Verified empirically 2026-07-20. Fix rides PENDING #51 (unify on the -files cwd-anchored matcher), which resolves it by construction; a subscription re-resolves on next subscribe. Also reachable from Lua (server.go:6559 sub.FilterFiles), so specs/lua-api.md moves with it.
- T260: R2133 retired by R3196 (2026-07-21 glob-unification #51)
- T261: R782 retired by R3204 (2026-07-21 glob-unification #51)
- T262: R783 retired by R3204 (2026-07-21 glob-unification #51)
- T263: R784 retired by R3204 (2026-07-21 glob-unification #51)
- T264: R1131 retired by R3204 (2026-07-21 glob-unification #51)
- T265: R1132 retired by R3204 (2026-07-21 glob-unification #51)
- T266: R1573 retired by R3204 (2026-07-21 glob-unification #51)
- T267: R1515 retired by R3204 (2026-07-21 glob-unification #51)
- T268: R942 retired by R3204 (2026-07-21 glob-unification #51)
- T269: R944 retired by R3204 (2026-07-21 glob-unification #51)
- [ ] O161: `ark unresolved` advertises `[PATTERN...]` in its help and ArgsUsage but `cmdUnresolved` never applies the patterns — it prints every unresolved path regardless. Pre-existing (predates #51), found 2026-07-21 while confirming which commands `filterPaths` actually serves; measured: `ark unresolved`, `ark unresolved '*.md'`, and `ark unresolved '/**/*.md'` all return the same 9601 lines. Related, and deliberately left alone by #51: `refresh`/`resolve`/`remove`/`dismiss` hand their patterns onward to be matched server-side (RefreshStale already uses db.matcher, so R3195 holds), which means those keep the rootless any-depth reading while `files`/`stale`/`missing` positionals are now cwd-anchored (R3208). Decide whether unresolved should filter (and if so, anchored like R3208) or whether the help should stop promising it.
- [ ] O162: `ark search --help` prints a hand-written `fs.Usage` from the handler body, while `ark help search` renders the node's urfave `Description` — two help texts for one command, the shape R2918 forbids. It is the last `SkipFlagParsing` node not routed through `helpRequested` + `ShowSubcommandHelp`; #51 fixed the other five (files, status, tag files, tag values, subscribe) and left this one alone because search's usage block is long-standing and hand-tuned, so collapsing it to one source is an editorial pass on the text, not a mechanical change. Found 2026-07-21 while verifying #51's help output.
- T270: R3174 retired by R3210 (2026-07-21 bible-xhtml #49 — verse identity read from XHTML ids, not backtick marks)
- [ ] O163: Book-index records are reconciled per *source* (R3221) but not per *file*: deleting a bible file, or an edition revision that drops a chapter from one file, leaves that (source, book, chapter) record pointing at a file that no longer holds it. Benign today — resolution reads the chunk's `chapter` attribute, so a stale entry lands on a file whose chunks do not match and returns nothing (R3180's answer anyway) — but it is a lie in the index. Fix by having the file-removal path drop the file's B records, which needs a value-side or reverse lookup the current key shape (source, book, chapter → path) does not support cheaply.
- [x] O164: R3219 says the per-source hook "fails the source's load" on a real BIBLE/ collision; activateSourceChunkers instead logs an ERROR and leaves the source indexed with its virtual namespace unregistered. Dropping the source from cfg.Sources — the literal reading — would make DiffConfig report a sources change against the stored I record on every boot and could trip the all-sources-gone catastrophe check, so the literal reading is unsafe as written. Decide between reconciling R3219's wording to "the source's bible addressing fails to load, loudly" and adding a persistent E record so `ark status` surfaces the collision rather than only the log.
- [ ] D13: R3223 conformance: `index_stale` is diff-derived, not witness-derived. Accepting a deferred change with `--force` writes the new value into the stored I record, so the next DiffConfig sees no difference and the condition clears itself — while the index is still wrong-shape, because nothing rebuilt it. Needs an artifact stamp instead: an I record holding the shape-determining config the index was actually built under (the Alibi Stamp pattern; `classifyField`'s ActionDefer set names the fields — `case_insensitive`, `chunkers`), written at rebuild and compared at load. Only a rebuild rewrites it, which is exactly the operation that repairs the damage. PENDING #55.
- [ ] D14: R3223 conformance: `ark serve --force` still calls ClearERecords (server.go ~234, ~249), clearing every condition including ones the user was never shown. R3223 supersedes that clause of R1558 — clearing is not a user-facing operation, and --force's job is accepting config changes. Blocked on D13 and D15: --force cannot stop clearing until each condition re-derives, or a real one would become unclearable. **Repairing this gap includes striking the clearing clause from R1558's text** — the requirement carries a forward note saying so, and leaving it would restore --force clearing on the next reader who takes R1558 at face value. PENDING #55.
- [ ] D15: R3223 conformance: two declared E conditions have no writer in production code. `ECondModelMismatch` appears only in store_test.go; `ECondConfigCatastrophe` has none at all — DiffConfig appends a ConfigChange whose field is `sources_catastrophe` (db.go ~2905), which is neither an E record nor the same name, and a comment at server.go:3449 asserts ReloadConfig writes a `sources_catastrophe` E-record, which it does not. specs/record-formats.md documents both as though they occur. Implement or retire each, and fix the stale comment. PENDING #55.
- [ ] O165: R3172 requires the bible strategy be mapped per-source (`[[source]].strategies`), but no CLI command writes that map: `ark config add-strategy` writes only the global `strategies` table (config.go AddStrategy) and takes no --source. So the one config shape the feature needs must be hand-edited into ark.toml, which also defeats the proxy-through-the-server rule for config mutation. Add --source to add-strategy (and to the /config/add-strategy handler), or a sibling verb.
- [x] O166: The rendered bible page shows no chapter number anywhere. renderBibleXHTML strips `chapter-num` as apparatus (R3211/R3183), which is right for the indexed text but leaves the reader unable to tell which chapter they are in — consecutive psalms run together separated only by a stanza gap, since editorial headings are dropped too (R3213). The chapter number is structure a reader navigates by, not reference apparatus like a footnote marker. Decide whether the renderer should emit it (as its own element, so it is display-only and still absent from chunk text) — the index/display split R3211 already draws makes that possible.
- [ ] O167: R3233's running head is browser behaviour (sticky positioning, scroll tracking, the body-overflow restoration) with no automated test — ark has no JS test harness, the same reason recorded in O95 for the tag-overview frontend. Verified by Playwright this session: the head pins at top 0, tracks forward and backward across chapters, and a non-bible markdown page gains neither the class nor the head. The Go half (the <ark-chapter> element itself) is covered by test-BibleRender.md.