...
- **R3071:** The accept loop closes by construction: `ark ext accept` (R3054) rewrites `@ext-candidate` → `@ext`; on reindex the RC derivation drops and the X + V edge lands, so committing a tag consumes its candidate with no separate bbolt step. `Store.AcceptDerived` (R2679) retires — it has no caller under the file-backed model.
- **R3072:** RF (`"RF" + chunkid`, freshness skip stamp) is unchanged. `runDerivationPass` still computes candidates and skips fresh chunks (R2669); only the survivor action changes (author a file tag instead of writing bbolt RC). RD and RM (session-scoped throttles) are untouched.
