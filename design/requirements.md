...
- **R3126:** `launch` (R3122) confirms the session came up using only content-free signals — never by parsing JSON record content. The protocol: (1) locate the new session's JSONL under `~/.claude/projects/<cwd-encoded>/` and wait for its **second record** to appear — a cheap early liveness gate that `claude` is connected to Claude Code, and a fail-fast if it never starts; (2) send the bootstrap to the pty (Claude Code buffers input, so ordering against startup is safe); (3) wait for the launched session to **claim the Luhmann seat** via `ark luhmann next --first` — the authoritative confirmation that `/luhmann` loaded and attached, and the event that teaches ark the session's id (from the claim's `--session`). A timeout on either wait fails the launch with an error. The seat claim is authoritative; the second-record gate is only a cheap early check. Content-based idle detection (classifying `origin.kind` / `turn_duration` / pending tool calls) is out of scope here — deferred until a consumer needs it.
