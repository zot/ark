...
- **R2894:** `SurfaceItem` (the `surface` verb) calls `Store.MarkSurfaced(session, chunkID)` for each surfaced chunk, starting the cooldown the moment a chunk is surfaced. The session is the result doc's originating session.
