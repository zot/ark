...
- **R3212:** A poetry stanza is one chunk. A stanza opens with a `<p class="line-group">` (or `line-group-after-heading` when it follows a title) and continues through the run of `line`, `line-indent`, and `line-space` paragraphs until the next stanza or prose block. A stanza is the reader's unit the way a paragraph is, so it holds together rather than fragmenting into a chunk per line.
