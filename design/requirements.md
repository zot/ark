...
- **R2749:** The curation doc body emits one `# Source Chunk: <jsonl-chunkid>` H1 per paragraph whose Recall result cleared `[recall].min_similarity`. Each H1 is followed by a `> `-quoted excerpt of the source paragraph (UTF-8-safe ~200-byte cap), then 1..K `## Candidate: <chunkid> <path>:<range>` H2s ordered by aggregate similarity score descending. Each candidate H2 carries three bullet lines — `- score: <0.NN>`, `- tags: <comma-separated bare tagnames>`, `- proposed-tags: <name> (<0.NN>), ...` (omitted when no surviving proposals) — and a fenced ~500-character excerpt of the chunk content.
...
- **R2751:** The result doc body emits zero or more sibling H2s in the order the recall agent emitted them. `## Surface: <chunkid>` recommends showing a chunk to the user — chunkID alone, since the assistant resolves path / range / context on demand via `ark chunks <chunkid>`. `## Recommend: @<tag>[:<value>] on <chunkid>` proposes a tag attach worth re-curating. Each H2 is followed by `reason: <one-line justification>` on the next line. When the agent emitted no surface or recommend items, no result doc is written (per R2758).
...
- **R2756:** `ark connections recall surface FIRE -chunk N -reason TEXT` appends exactly one `## Surface: <chunkid>` H2 (chunkID alone) to the result-doc builder for FIRE, implicitly opening the builder on first call. One item per invocation. Missing or empty required flag values exit non-zero before any state mutation. The chunkID alone identifies the surfaced chunk; path / range / context are resolved on demand via `ark chunks <chunkid>`.
...
- **R2763:** `~/.ark/monitoring/recall.jsonl` is an append-only log written by `close`. Each line is one JSON object with fields `fire` (int), `session` (string), `nonce` (int), `in_tokens` (int), `out_tokens` (int), `latency_ms` (int), `surfaced` (int), `recommended` (int), `outcome` (string, one of `"result-emitted"`, `"silent-close"`, or `"error"`), `timestamp` (RFC3339 string). The format is forward-compatible — future fields slot in at the end.
