...
- **R3018:** `[luhmann].class.<NAME>.cooldown_seconds` (int, default `120`) is how long a pooled-class secretary that has returned to idle stays warm before it is eligible for a `stop one` directive (damps spawn/stop churn).
...
- **R3026:** Curation is mandatory (every CLI finding gets Luhmann's judgment) but decoupled from occupancy — the secretary was already freed at the return (R3024), so curation costs only the CLI's own latency, never a held pool slot. Luhmann's opt-in to serve CLI curation is simply owning the `next` seat (`--first`, R3013); there is no separate parent-signal subscription — the ownership lease is the opt-in, and a session not draining `next` serves no CLI hunts.
- **R3027:** `ark bloodhound add --result tmp://BLOODHOUND-CLI/<id> --id CHUNK-ID --path PATH --chunk CHUNK-TEXT` is Luhmann's result stencil: each call appends one JSON line to the **result** doc, one item per call (Stencil pattern, the discipline of `surface` / `recommend` / `finding`); a terminal call flips the result doc's tag to `@ark-bloodhound-cli-result: <id>`, the notification the waiting CLI is subscribed to. The pool secretary's raw results go instead to the **request** doc via the existing bloodhound builder path (R2943/R2945), re-tagging it to `@ark-bloodhound-cli-return` — two write targets, two docs.
