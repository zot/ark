...
- **R1246:** For tag mode (Phase A): step 2 is trigram fuzzy matching against V records (the tag-value index) **Slated for retirement — see gap O140** (spectral→bloodhound teardown); still live today, so do not invest in it without checking that gap first.
- **R1270:** Haiku expand step: given user's tag name and value, suggests alternative tag names and values **Slated for retirement — see gap O140** (spectral→bloodhound teardown); still live today, so do not invest in it without checking that gap first.
- **R1271:** Fuzzy match step: each alternative is fuzzy-matched against V records, producing (tag, value, count, score) tuples **Slated for retirement — see gap O140** (spectral→bloodhound teardown); still live today, so do not invest in it without checking that gap first.
- **R1272:** Haiku curate step: sees matched tag/value pairs with scores, prunes false positives, returns curated subset **Slated for retirement — see gap O140** (spectral→bloodhound teardown); still live today, so do not invest in it without checking that gap first.
- **R1273:** Server fetches actual search results for the curated tags before returning to the client **Slated for retirement — see gap O140** (spectral→bloodhound teardown); still live today, so do not invest in it without checking that gap first.
...
- **R1378:** Curation endpoints are renamed from `/search/expand` to `/search/curate` — curation is now a separate step from expansion **Slated for retirement — see gap O140** (spectral→bloodhound teardown); still live today, so do not invest in it without checking that gap first.
- **R1379:** `POST /search/curate` queues a curation request (replaces `POST /search/expand`) **Slated for retirement — see gap O140** (spectral→bloodhound teardown); still live today, so do not invest in it without checking that gap first.
- **R1380:** `GET /search/curate/wait` is the lotto tube for the sidecar (replaces `GET /search/expand/wait`) **Slated for retirement — see gap O140** (spectral→bloodhound teardown); still live today, so do not invest in it without checking that gap first.
- **R1381:** `POST /search/curate/result` receives sidecar results (replaces `POST /search/expand/result`) **Slated for retirement — see gap O140** (spectral→bloodhound teardown); still live today, so do not invest in it without checking that gap first.
- **R1382:** `GET /search/curate/result/{id}` polls for a curation result (replaces `GET /search/expand/result/{id}`) **Slated for retirement — see gap O140** (spectral→bloodhound teardown); still live today, so do not invest in it without checking that gap first.
- **R1383:** Expansion and matching endpoints remain under `/search/expand/`: fuzzy, embed, search **Slated for retirement — see gap O140** (spectral→bloodhound teardown); still live today, so do not invest in it without checking that gap first.

### Availability
- **R1248:** Server checks for `claude` on PATH at startup **Slated for retirement — see gap O140** (spectral→bloodhound teardown); still live today, so do not invest in it without checking that gap first.
- **R1249:** `GET /status` includes `spectral: true/false` capability flag **Slated for retirement — see gap O140** (spectral→bloodhound teardown); still live today, so do not invest in it without checking that gap first.
- **R1250:** (inferred) The check is a one-time `exec.LookPath("claude")` at startup, not per-request **Slated for retirement — see gap O140** (spectral→bloodhound teardown); still live today, so do not invest in it without checking that gap first.

### Searching Directory
- **R1251:** `~/.ark/searching/CLAUDE.md` contains the system prompt for the Haiku expansion session **Slated for retirement — see gap O140** (spectral→bloodhound teardown); still live today, so do not invest in it without checking that gap first.
- **R1252:** `ark init` creates `~/.ark/searching/` and writes a default `CLAUDE.md` if the directory doesn't exist **Slated for retirement — see gap O140** (spectral→bloodhound teardown); still live today, so do not invest in it without checking that gap first.
...
- **R3223:** An E condition may only be recorded if it is **re-derivable from durable state that witnesses it**. Each config load recomputes the conditions it owns and reconciles the records to that answer — present conditions written, absent ones deleted — so a record cannot outlive the problem it describes (the mechanism behind R1545) and no condition needs dismissing. The witness must be chosen for what the condition is *about*: comparing loaded config against stored I records compares intent to intent, so a condition describing damage to a **built artifact** must instead be witnessed by a stamp on that artifact, carrying the config it was built under. Where a witness makes self-repair possible, prefer repairing to reporting (`ec_version` drops and regenerates EC records at startup without ever surfacing a condition); an error condition is what remains when the repair needs a human. Two consequences: clearing is not a user-facing operation, so `--force` must not clear records (superseding that clause of R1558 — its job is accepting config changes); and a declared condition with no writer must be implemented or retired rather than documented as though it occurs.
...
- **R1558:** `--force` on `ark serve` clears E records, accepts current config, updates all I records, applies fix-minimal where applicable. **The clearing clause is superseded in principle by R3223 — see gap D14** and survives only because the other conditions do not re-derive yet: strip the clause when that gap is repaired (which is itself blocked on D13 and D15 — until every condition re-derives, removing `--force` would leave a genuinely stuck condition unclearable). The rest of the requirement stands on its own.
...
- **R3172:** A `bible` chunking strategy is registered under that name and is selectable **per-source** (e.g. `[[source]] strategies = { "**/*.text.xhtml" = "bible" }`) — so a scripture checkout opts in by config with no other change. The mapping is local to the source rather than global: a global bible rule has no particular source to set up for, and the per-source mapping is what triggers the chunker's source-specific setup (R3217).
- **R3173:** The bible chunker emits **one chunk per block**, where a block is a prose paragraph (`<p class="normal">`, or `no-indent` at a chapter start) or a poetry stanza (R3212). A prose paragraph runs across several verses as a reader sees them — a paragraph opening at verse 3 holds verses 3–5, and the next opens at verse 6. Block granularity is the deliberate middle between a chapter (too coarse to be a useful search result or annotation target) and a verse (too fine — an order of magnitude more chunks to serve an addressing need that the chunk's verse range and display-time refinement satisfy for free). Replaces the earlier markdown blank-line-paragraph rule.
- **~~R3174:~~** (Retired T270 — see R3210) A verse mark is a **backtick-wrapped integer** (`` `12` ``) appearing inline in the prose. Backticks are what make the mark unambiguous — nothing else in the text can be mistaken for one, and ordinary markdown viewers render it as a small code span. A verse begins at its mark and runs to the next; verses are prose-sized, so several share a paragraph and one may continue past a paragraph break.
- **R3175:** Each chunk carries a `chapter` attribute holding the chapter number of the `<section epub:type="chapter">` the block sits in, read from the block's `vBBCCCVVV` ids (`v01003001` = chapter 003) rather than recognized from heading text. It is absent on any block preceding the file's first verse-bearing element (a preamble or title).
- **R3176:** Each chunk carries a `verses` attribute spanning the verses the block covers — `first-last` (`"3-5"`), or the bare number when the block holds one verse — read from the `vBBCCCVVV` ids of its verse-bearing spans. The **range**, not merely the first verse: R3180 must reject a verse past a chapter's last block, and only the range end distinguishes a verse within the block from one beyond it. Together with `chapter` this is all a reference needs to find its paragraph; no verse numbers enter the chunk text (R3211).
- **R3177:** The book name is deliberately **not** recorded as a chunk attribute. The file already names it (the epub filename token) and a reference addresses a file, so storing it per-chunk would duplicate an identity that cannot disagree with itself. The book name lives only in the book-index key (R3214), which maps it to the file.
...
- **R3179:** An `@ext` TARGET may address a bible file by **chapter and verse** in the anchor slot where a line range would otherwise sit: `<file>:12.1` means chapter 12, verse 1. It resolves to the single paragraph chunk whose `chapter` matches and whose `verses` span contains that verse. The friendly virtual form `<source>/BIBLE/<Book>:12.1` reaches the real file first via the book index (R3216), then resolves here.
- **R3180:** A reference naming a book, chapter, or verse that does not exist resolves to **nothing** — no chunks. It is not an error, and it does not fall back to the file's first chunk the way a bare path target does, because a reference that named a specific verse and silently annotated an unrelated paragraph would be worse than one that annotated nothing.
- **R3181:** A bible file's content view renders by **intermediating its XHTML** through a bible-specific renderer rather than the shared markdown one. Each `verse-num` span becomes `<ark-verse n="N">` wrapping the number, giving the verse an identity in the page. **Every** verse mark is wrapped, not only annotated ones: a verse is the unit a reader refers to, so scroll-to-verse, highlighting, and annotate-this-verse affordances need a target before anything is attached. The front end draws a small gold tag icon after a verse number where a routing is placed.
- **R3182:** An `@ext` routing whose target names a verse (`…:12.1`) is rendered **inside that verse's `<ark-verse>` element**; routings targeting the same chunk that name no verse — a bare path, a quoted-text or regex anchor — remain in the chunk-level `<ark-ext-tags>` block where every other content kind shows them. The partition is by whether the routing's recorded target anchor parses as CHAPTER.VERSE (R2073 supplies the anchor). Nothing is dropped for lacking a verse and nothing is invented for having one.
- **R3183:** The rendered page is produced by **transforming the publisher's XHTML into ark's own controlled elements**, not by serving the file's markup and not by rewriting output. The publisher's XHTML carries inline event handlers (`onclick`), a `<script>` reference, external stylesheet links, and footnote/crossref popups — serving it directly is the raw-HTML injection ark refuses for every indexed file. The apparatus is stripped and the scripts and handlers dropped; ark emits only its own elements, so a bible page is exactly as safe as any content page. Verse recognition is over the parsed document, where a `verse-num` span is structurally distinct from any number in the prose, so only real verse marks become `<ark-verse>` and sentence text is untouched.
- **R3209:** Only `*.text.xhtml` files are indexed and rendered by the bible strategy. An epub extracts each book into a text file plus siblings — `*.main.xhtml` (book intro) and `*.crossrefs.xhtml` / `*.footnotes.xhtml` / `*.resources.xhtml` (apparatus) — which are neither chunked nor rendered.
- **R3210:** Verse identity is read from the publisher's XHTML markup, not recognized from the prose. Every structural element carries its book, chapter, and verse in a stable id and class (`id="v01003001"`, `class="h01003001"` = book 01, chapter 003, verse 001), so the chunker reads the publisher's answer rather than matching a mark in the text. Replaces R3174 (backtick marks).
- **R3211:** A chunk's text is prose only. The `verse-num`, `chapter-num`, and `book-name` spans and the footnote/crossref apparatus are stripped, leaving the sentence a reader reads — which is what the trigram index and the embedder see and nothing else.
- **R3212:** A poetry stanza is one chunk. A stanza opens with a `<p class="line-group">` (or `line-group-after-heading` when it follows a title) and continues through the run of `line`, `line-indent`, and `line-indent2` paragraphs until the next stanza or prose block. A stanza is the reader's unit the way a paragraph is, so it holds together rather than fragmenting into a chunk per line. **`line-space` is not a continuation**, despite matching a `line` prefix: the edition gives it a top margin and nothing else, so it keeps the base paragraph's first-line indent while every genuine poetry class overrides `margin-left` and `text-indent`. Reading it as poetry merged 150 of its 151 occurrences into the stanza above (Genesis 1:28's prose into the poetry of 1:27). Class membership is therefore decided by what the edition *styles*, not by what the class name begins with.
- **R3213:** Editorial headings (`<header><p class="heading">` — the publisher's pericope titles) are dropped from the index and the rendered page by default. They are an interpretation imposed on the text rather than the text itself, and indexing them would let a search match an editor's phrasing as if it were scripture. A per-source option to show them (which would then also index them, so page and index never disagree) is not built until a corpus wants it.
- **R3214:** The chunker writes a book-index record at index time, one per (source, book, chapter): key `B <source> \0 <book> \0 <chapter>`, value the file path. NUL bytes delimit the variable-length fields (a literal `0` would collide with digits in a chapter or path); the source is in the key so two scripture sources cannot collide on the same book and chapter. Chapters do not span files, so the entry is an exact-key lookup, not a range scan. The record class is added to `specs/record-formats.md` and given a `status -db` label.
- **R3215:** The book name in the key and the address is the epub's filename token with each hyphen turned to a space (`1-Samuel` → `1 Samuel`, `Song-of-Solomon` → `Song of Solomon`), spelled as the edition spells it (`Psalm`, singular). Aliases (`Psalms`, `Ps`) are not part of the first cut.
- **R3216:** The friendly address `<source>/BIBLE/<Book>:<chapter>.<verse>` is a **virtual path** — no file exists at it. It resolves in two stages: `BIBLE/<Book>` plus the anchor's chapter looks up the book index (R3214) to the real file, then `:chapter.verse` resolves to the paragraph chunk (R3179). `BIBLE/` is one reserved segment (rather than 66 root-level book names, which are common words that would silently shadow real files); it is address syntax, not part of the DB key. The real file path stays addressable directly and resolves to the same chunk.
- **R3217:** An optional Go interface is called on a chunker when it is mapped by a **per-source** strategy, once per source at config-resolve time (every startup). A chunker mapped only by the global strategy map is not called — a global mapping has no particular source to set up for. The interface receives the source and a handle to register strategy entries and may return an error to fail the source's load. A chunker that does not implement the interface is unaffected.
- **R3218:** In that hook the bible chunker registers a source-prefixed strategy entry `<source>/BIBLE/** → bible` into the **in-memory global strategy map**. The source prefix makes a global-map entry safe — it can only match that source's paths — and matching is against the file's **absolute** path (the filesystem-absolute glob form, R3196). This classifies the virtual `BIBLE/<Book>` addresses as bible so they dispatch to the bible resolver with no bespoke branch, and it is re-established every startup rather than persisted (the book index is the only bible data on disk).
- **R3219:** The bible chunker's per-source hook **fails to activate if a real `<source>/BIBLE` path exists**, so the reserved virtual namespace cannot collide with real content, and the source's virtual addressing is left unregistered. The check is scoped to bible sources, since only they run the hook. The failure is announced two ways, because it is otherwise silent — the source keeps indexing and only its `BIBLE/` addresses stop resolving, which reads as a reference that returns nothing: ark logs it naming the offending path, and records a persistent error condition (`source_activation`) that `ark status` reports. The source itself is **not** removed from the configuration: dropping it would make `DiffConfig` report a sources change against the stored I record on every boot and could trip the all-sources-removed catastrophe check, so a misconfigured source would corrupt unrelated config state. The condition is re-derived on every config load — written when any source fails, deleted when none do — so fixing the collision and reloading clears it with no dismissal step.
- **R3220:** Target-anchor decoding is dispatched by strategy: the bible resolver owns the whole bible address path — recognizing the virtual `BIBLE/` form and rewriting it via the book index, then decoding `chapter.verse` — rather than the generic resolver carrying a bible-specific branch that assumes a real indexed file.
- **R3222:** A verse whose markup carries no `verse-num` span still gets an `<ark-verse n="N">` element — an empty anchor emitted where its text begins. This is the first verse of every chapter: the edition sets a large `chapter-num` in place of the verse number, so the verse has identity in the markup (`hBBCCCVVV`) but no number to wrap. Without the anchor it would be the one verse per chapter a reader cannot address, and a routing targeting it (R3182) would resolve to the right paragraph and then have no element to render in — dropped silently. The rendered page is unchanged; only the addressability differs. Serves R3181's "every verse is wrapped" where its literal reading ("every verse *mark*") does not reach.
- **R3221:** At config-resolve, every book-index record whose source no longer declares the bible strategy is deleted. The book index is the only bible data on disk (R3218), so it is the only part that can outlive the configuration that produced it: a source that drops the strategy, or is removed from `ark.toml`, stops being indexed, so no re-index pass ever reaches its records and a later virtual lookup would resolve through a stale entry to a file ark no longer knows. The sweep runs in the same per-source pass that registers the virtual namespace (R3217), which is where the set of scripture sources is already decided, and it therefore also runs when *no* source declares the strategy — the source-removed case. **Pending extension — see gap O163**: reconciliation is per *source*; per-*file* staleness is not yet covered.
- **R3224:** Only blocks inside a `<section epub:type="chapter">` are chunked. A `*.text.xhtml` file carries its own footnote and cross-reference popups and its navigation templates appended after the scripture, so excluding the sibling apparatus files (R3209) does not exclude the apparatus: measured on the 181-book ESV corpus, 6,382 of 13,887 chunks were `note`, `nav`, and `nav-header` boilerplate. The containment rule is chosen over a list of apparatus class names because a list must be re-derived per edition and fails silently when incomplete (unlisted apparatus reads as scripture), whereas containment fails toward an empty book, which is noticed. It is corroborated by a second, independent signal: the appended material sits in `<section class="hidden">` and `<div class="hide">`, which the edition's own stylesheet sets to `display: none`. Across the corpus the two signals partition every `<p>` class identically, with no class straddling.
- **R3225:** Scripture is not identified by whether a block carries a verse identity. A block continuing a verse that opened in an earlier block has no id of its own (Exodus 34:26b), and neither does a psalm superscription nor a Psalm 119 acrostic letter title; all are read text. On the ESV corpus that rule would have discarded 157 genuine blocks while catching nothing R3224 does not already catch.
- **R3226:** The page is styled from the publisher's own stylesheet, translated onto ark's classes: a verse number renders small, bold, and raised; a prose paragraph indents its first line except at a chapter opening, where the indent is suppressed for the chapter number; a poetry line hangs, with the stanza inset and each line's first row starting left of that inset, a deeper line class inset further, and a blank line above each stanza. An unstyled page is not merely plain but wrong — an inline body-size verse number reads as part of the sentence (`2The earth was`), and unstyled poetry loses the indentation that is the sole reason per-line structure is carried through a stanza chunk.
- **R3227:** The publisher-class → ark-class map preserves every distinction the presentation depends on. Collapsing two publisher classes into one ark class discards whatever the edition's stylesheet said differed between them, silently: folding `no-indent` into `normal` loses the suppressed chapter-opening indent (2,060 paragraphs), and folding `line-group` into `line` loses the blank line separating one stanza from the next (1,923 stanzas), which is what R3212 makes a stanza its own chunk to preserve. A class ark does not map still falls back to prose, so an unseen edition reads.
- **R3228:** The gold tag icon marking an annotated verse renders **inline**, after the verse number, as a variant of the chunk-level `<ark-ext-tags>` indicator rather than that indicator unchanged. The chunk-level form floats left to sit in a chunk's margin; inside an `<ark-verse>` mid-sentence a float would pull the icon to the paragraph's left edge, away from the verse it marks.
- **R3229:** `<ark-verse>` carries its chapter as well as its verse number. A text file spans several chapters (Genesis 1 and 2 share one), so a verse number alone does not identify a verse within a page. The renderer reads both from the same `vBBCCCVVV` identity, so the chapter costs no additional parsing. Annotation placement (R3182) is unaffected and keys on verse number alone, since a routing is placed within the chunk it resolved to and a chunk never spans chapters.
- **R3230:** A verse is reachable in the page by chapter and verse: the content view scrolls that verse into view and marks it briefly. R3181 wraps every verse to give scroll-to-verse, highlighting, and annotate-this-verse a target; this is the first of the three, and until one exists the every-verse rule and R3222's numberless anchor pay a cost nothing collects.
- **R3232:** The chapter number is rendered, as `<ark-chapter n="N">`, at the head of the chapter's opening paragraph and set large as the edition sets it. It is **display only** — the chunker still strips it, so R3211's prose-only indexed text is unchanged and no search can match a chapter number. Stripping it from the *page* too (gap O166) over-applied the apparatus rule: a footnote marker is reference apparatus a reader can lose, but a chapter number is the structure a reader navigates by, and without it a scripture page carries nothing saying where the reader is. The element also carries the edition's book label as an attribute where the markup supplies one, which is at every chapter opening; the label is not repeated in the running text.
- **R3233:** The page carries a **running head** naming the book and chapter currently in view, which stays visible as the reader scrolls and updates on crossing into the next chapter. A chapter number is only visible while the top of its chapter is, which on a file holding eleven chapters leaves most of the page unlabelled. It is a single page-level element kept current by the page rather than a `position: sticky` element per chapter: the rendered page is a flat sequence of per-chunk containers (one per paragraph or stanza, carrying that chunk's annotations and controls), so a chapter has no container of its own for a sticky child to travel the length of.
- **R3231:** A bible file's content view presents no edit affordance. R3178 states this as a consequence of non-writability, but the toggle is suppressed only by an explicit query parameter, so the pencil is drawn on a bible page and opens the publisher's XHTML in the markdown editor with a save action. The content view suppresses it for a non-writable strategy rather than relying on the caller to ask.
