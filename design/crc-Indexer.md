...
- AddFile(path, strategy): add to microfts2 (gets fileid + chunk offsets),
  ...
  extract tag values, update Store V records (R1103, R1106)
...
- RefreshFile(path): check for append-only change first. If append:
  ...
  old vectors, add new vectors, re-extract tags, tag defs, and tag
  values, update Store (including V records). (R1103)
...
- AppendFile(path, fileid, strategy): read new bytes from FileLength
  ...
  Store.AppendTags, Store.AppendTagDefs, and Store.AppendTagValues
  for incremental update. (R1104)
