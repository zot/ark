# PtyBrowser
**Requirements:** R3141, R3142, R3143, R3144
...
to the CLI's unix-socket transport. The xterm.js terminal that drives it is a
later `/ui-thorough` slice; this card covers only the Go endpoint and its wire.
...
- route registration: `GET /luhmann/pty` is wired in `registerContentRoutes` (crc-Server.md) on the UI HTTP server, so it exists whenever the UI runtime is running (R3141).
...
- browser xterm.js client: the later `/ui-thorough` consumer that opens the socket, sends resize / repaint / input, and renders output. Connected-to, not part-of.
