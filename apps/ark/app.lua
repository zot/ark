...
local Curation, PinnedChunk, TagSuggestion, HotChunk, RelatedTag, DriftPair
...
function Ark:showCuration()
    self._viewMode = "curation"
...
function Ark:curate(chunkID, fileID, path)
    self._curation:curate(chunkID, fileID, path)
end
...
------------------------------------------------------------------------
-- Ark.Curation (vocabulary-maintenance workshop)
------------------------------------------------------------------------

local CURATION_DIR = HOME .. "/.ark/apps/curation"
local CURATION_STATE_PATH = CURATION_DIR .. "/state.json"
local SUGGESTIONS_K = 8
local TOP_CHUNKS_K = 12
local RELATED_TAGS_K = 8

local function loadCurationState()
    local f = io.open(CURATION_STATE_PATH, "r")
...
local function saveCurationState(data)
    os.execute('mkdir -p "' .. CURATION_DIR .. '"')
    local f = io.open(CURATION_STATE_PATH, "w")
...
function Curation:new(instance)
    ...
    instance.pinned = instance.pinned or {}
...
function Curation:mutate()
    if self.pinned == nil then self.pinned = {} end
...
function Curation:_load()
    local data = loadCurationState()
    self._stateLoaded = true
    if not data then return end
    self._lastViewedAt = tonumber(data.lastViewedAt) or 0
    local pins = data.pinned
    if type(pins) ~= "table" then return end
    -- Keep order from disk: top of list = newest
    self.pinned = {}
    for _, p in ipairs(pins) do
        local pin = session:create(PinnedChunk)
        pin.chunkID = tonumber(p.chunkID) or 0
        pin.fileID = tonumber(p.fileID) or 0
        pin.path = p.path or ""
        pin.pinnedAt = tonumber(p.pinnedAt) or 0
        if pin.chunkID ~= 0 and pin.path ~= "" then
            table.insert(self.pinned, pin)
        end
    end
end

function Curation:_persist()
    local pins = {}
    for _, p in ipairs(self.pinned) do
        table.insert(pins, {
            chunkID = p.chunkID,
            fileID = p.fileID,
            path = p.path,
    ...
    saveCurationState({
        lastViewedAt = self._lastViewedAt,
        pinned = pins,
    })
end

function Curation:_findPinIndex(chunkID)
    for i, p in ipairs(self.pinned) do
        if p.chunkID == chunkID then return i end
    end
    return nil
end
...
function Curation:curate(chunkID, fileID, path)
    ...
    fileID = tonumber(fileID) or 0
    path = path or ""
    if chunkID == 0 then return end
    local existingIdx = self:_findPinIndex(chunkID)
    local pin
    if existingIdx then
        pin = table.remove(self.pinned, existingIdx)
        pin.fileID = fileID ~= 0 and fileID or pin.fileID
        if path ~= "" then pin.path = path end
    else
        pin = session:create(PinnedChunk)
        pin.chunkID = chunkID
        pin.fileID = fileID
        pin.path = path
    end
    pin.pinnedAt = nowSeconds()
    pin._newSinceLastView = true
    pin._suggestionsLoaded = false
    pin._suggestions = {}
    pin._suggestionsError = ""
    table.insert(self.pinned, 1, pin)
    self:_persist()
end

function Curation:dismissChunk(pin)
    if not pin then return end
    local idx = self:_findPinIndex(pin.chunkID)
    if not idx then return end
    table.remove(self.pinned, idx)
    self:_persist()
end

function Curation:sweepOlder()
    if #self.pinned <= 1 then return end
    local top = self.pinned[1]
    self.pinned = {top}
    self:_persist()
end
...
function Curation:newCount()
    local n = 0
    for _, p in ipairs(self.pinned) do
        if p._newSinceLastView then n = n + 1 end
    end
    return n
end
...
function Curation:onViewOpen()
    -- Mark which pins are new relative to the previous view-open,
    -- then advance the timestamp so subsequent pins look new again.
    local prev = self._lastViewedAt
    for _, p in ipairs(self.pinned) do
        p._newSinceLastView = (p.pinnedAt > prev)
    end
...
function Curation:isFocused()
    return self.focusedTag ~= ""
end

function Curation:notFocused()
    return self.focusedTag == ""
end

function Curation:focusError()
    return self._focusError
end

function Curation:noFocusError()
    return self._focusError == ""
end

function Curation:focusedChunkCount()
    return #self._focusedChunks
end

function Curation:focusedRelatedCount()
    return #self._focusedRelated
end

function Curation:focusedDriftCount()
    return #self._focusedDrift
end
...
function PinnedChunk:dismiss()
    if curation then curation:dismissChunk(self) end
end

function PinnedChunk:openFile()
    if self.path ~= "" then mcp.open(self.path) end
end

function PinnedChunk:isNew()
    return self._newSinceLastView
end

function PinnedChunk:notNew()
    return not self._newSinceLastView
end
...
function HotChunk:pin()
    if curation then curation:curate(self.chunkID, self.fileID, self.path) end
end

function HotChunk:openFile()
    if self.path ~= "" then mcp.open(self.path) end
end
...
function RelatedTag:focus()
    if curation then curation:focusTag(self.tag) end
end
