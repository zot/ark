...
| 🗄 archive-fill | displayArk() | Ark Search |
| ✉ envelope-fill | displayArkMessages() | Ark Messenger |
...
| displayArk() | Switch to ark app in Searching mode (calls `ark:showSearching()` then `mcp:display("ark")`) |
| displayArkMessages() | Switch to ark app in Messaging mode (calls `ark:showMessaging()` then `mcp:display("ark")`) |
