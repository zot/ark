...
| `kind`     | string | One of `spawn`, `exit`, `respawn`, `crash`, `pause`, `resume`.                                                                                                         |
...
| `reason`   | string | For `exit` and `crash`, a short classification string (`context-limit`, `error`, `external-pause`, etc.). Empty otherwise.                                             |
| `crashes`  | int    | The consecutive-crash counter at the time of the record. Resets to 0 on a healthy `exit`.                                                                              |
...
- `--reason context-limit` → `kind: "exit"` (healthy recycle).
  `crashes` resets to 0.
- Any other reason → `kind: "crash"`. `crashes` increments from
  the previous value in the log; `--crashes K` overrides the
  computed value when the caller has already classified.

Called by the orchestrator session when it observes a managed
subagent's Task complete. The companion `respawn` record (if the
supervisor decides to respawn) is written by the next `spawn-record`
...
`healthy`, `crash`, or `unknown`. Output:

- Default: one of the three labels on stdout.
...
- `healthy` — the most recent record in the subagent JSONL is a
  `tool_result` for `ark connections recall close` *or* the
  matching `recall.jsonl` entry shows `outcome: "closed"`.
...
| `crash_pause_after`              | int      | `3`           | After this many consecutive crashes for a class, the supervisor stops respawning and writes a `pause` record. User clears with `ark monitor resume`. |
