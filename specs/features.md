...
| Luhmann orchestrator (supervisor + chimes)  | Phase 2 in spec (Go-side `ark monitor` / `ark luhmann` CLI, `[luhmann]` config, `@chime-Nm` standard scheduling tags, subscriber-presence gate). Skill / agent files separate. One drain tube (`ark luhmann next`) now carries every producer: curation tasks + supervisor directives (watcher), commands (`ark luhmann send`, synchronous), and Frictionless UI events (`ark luhmann events` opt-in, shipping — single reader, gates `ark ui event`, no inheritance across seats). | `luhmann.md`, `monitor.md`, `chimes.md`, `subscriber-presence.md`, `.scratch/LUHMANN-ORCHESTRATOR.md` (working notes) |
| Managed PTY session (ark hosts Luhmann)     | Phase 1 shipping (Go CLI pty core: `ark luhmann launch`/`attach`/`status`/`stop`, fan-out multiplexer, content-free launch confirmation). Phase-2 browser transport shipping (Go `GET /luhmann/pty` websocket, ark's own `gorilla/websocket`, binary pty bytes + JSON resize/repaint control frames, plus a `/luhmann/pty-status` UI-mux mirror so a browser can tell "asleep" from "bouncing"). Phase-2 browser *client* shipping: the `<luhmann-terminal>` xterm.js element + its standalone page — the browser's `ark luhmann attach`. Its placement in the Frictionless app (panel, desk-lamp, listener toggle) stays deferred to the phase-3 `/ui-thorough` slice (#39). | `managed-pty.md`, `luhmann-terminal-element.md`, `.scratch/LUHMANN-MANAGED-SESSION-20260709.md` (working notes) |
