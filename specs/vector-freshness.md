...
write with the transaction's serial so corpus-wide derived
...
Language: Go. Environment: ark server, the index via
`go.etcd.io/bbolt`.
...
by a per-record stamp. Each write transaction draws a monotonic
serial from a maintained counter. Stamping every record written in a
txn with that serial gives a per-transaction monotonic mark: records
written together share a serial, records written later have a strictly
greater serial.
...
(`I:serial`). Because it sits in the live B-tree, it gets preserved
by every compaction and is monotonic across the database's entire
lifetime, independent of any engine-internal transaction bookkeeping.
...
plus one extra `b.Put` per stamped record (writing the S-entry),
...
`S*` entry in the **same transaction**:
...
func stampWrite(txn *bbolt.Tx, prefix, key []byte) error
```
...
after their existing `b.Put` for the value.
...
All records stamped within one write transaction share a single
...
- **Crash mid-write** is handled by the index's transactional guarantees:
  either the value write and the S-entry both land, or neither does.
  No half-stamped records.
- **Compaction at startup** (`Tx.WriteTo`)
  preserves the I-record counter and every S-record because both
  live in the active B-tree. The counter never resets. A future
  enhancement may renumber serials during compaction (subtract the
  minimum serial from every S record, reset the counter to
  `max - min + 1`) to keep varint widths small over many compactions;
...
(1 byte 'S' + 1-2 prefix bytes + key tail) + B-tree node
...
- **Write path (single record):** one I-record read + one I-record
  write to allocate the serial, plus one `b.Put` for the S-record,
  on top of the existing value put. The I-record is small (single
  key, single B-tree page) and stays in the index's page cache after the
  first access. At a typical 5–20 μs per put, single-record
  writes pay roughly one extra put's worth of latency.
- **Write path (batch):** one allocation up front, then one extra
  `b.Put` per record (the S-stamp). A 256-chunk batch is one
  alloc + 256 value puts + 256 stamp puts, all in one txn.
- **Read path (RecordSerial):** one `b.Get` against the S-side
