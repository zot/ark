...
ark luhmann send "<instruction>" [--timeout SECONDS]
...
| `next`           | `--session` (required identity); `--first` (claim if unowned), `--force` (reclaim); `--keepalive SECONDS` (idle window, default ~2700 = 45m, capped under the 1h cache) | Server-required. The orchestrator's blocking drain tube: returns one curation task, supervisor directive, command, frictionless-event (when routing is on, see `events`), or keepalive crank-handle, guarded by an in-memory ownership lease. `--first`/plain/`--force` per the lease; a foreign owner exits 2 (stand down), an unowned server on a plain call returns the reclaim crank-handle. Backgrounded + re-invoked in a loop; subsumes the standalone `ark heartbeat` keepalive. |
| `send`           | `"<instruction>"` (required positional); `--timeout SECONDS` (wait bound, default 120) | Server-required. Synchronous producer for the drain tube: enqueues the instruction as a `command`-kind work item with an inert backquoted correlation nonce (Watermark), waits for the orchestrator to handle it, then prints the orchestrator's turns for that instruction — bracketed from the nonce's appearance in the session JSONL to the first turn completion — rendered like `ark chats --with-tools`. Errors orchestrator-not-running when no session owns the tube; on `--timeout` exits non-zero (the enqueue stands). |
| `events`         | `--session` (required identity); `--off` (release) | Server-required. Route Frictionless UI events onto this session's `next` tube as a `frictionless-event` kind. Routing is a privilege of the `next` seat: a session not owning it errors `you don't have ownership`. While owned, `ark ui event` errors 409 — the drain is destructive, so exactly one reader is served. `--off` releases and restores `ark ui event`. Routing does not inherit: a seat change or a bounce clears it, so a new orchestrator must ask for itself. |
...
| `event` | `GET /wait?timeout=120` — block until next UI event; retries on transient. Errors (409, exit 1) while a Luhmann orchestrator owns event routing (`ark luhmann events`), which reroutes these events onto its own tube — not retryable, since only that owner releases it. Unaffected when no orchestrator has asked. |
